...
def initCamera(dll_path, xml_path, qtWidget, manual=False):
    """初始化相机, 传入qt的窗口handle
    返回ic对象
    """
    # ic 对象初始化
    ...
    # 打开相机
    if manual:
        camera_handle = ic.IC_ShowDeviceSelectionDialog(None)
    else:
        camera_handle = ic.IC_LoadDeviceStateFromFile(None, T("./device.xml"))
    
    if ic.IC_IsDevValid(camera_handle):
        ic.IC_SetHWnd(camera_handle, qtWidget.winId())
        ic.IC_StartLive(camera_handle, 1)
